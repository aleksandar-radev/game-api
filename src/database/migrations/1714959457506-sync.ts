...
export class Sync1714959457506 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE UNIQUE INDEX "IDX_USER_EMAIL" ON "app1"."users" ("email") `
    );
    await queryRunner.query(
      `CREATE UNIQUE INDEX "IDX_USER_USERNAME" ON "app1"."users" ("username") `
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "app1"."user_data" DROP CONSTRAINT "FK_USER_DATA_USER_ID"`
    );
    await queryRunner.query(
      `DROP INDEX "app1"."IDX_USER_DATA_USER_ID_PREMIUM"`
    );
