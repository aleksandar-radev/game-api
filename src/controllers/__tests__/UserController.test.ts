...
describe('UserController', () => {
  ...
  async function createAndLoginUserAndReturnUserAndToken() {
    ...
    const loginResponse = await request(server).post('/api/user/login').send(loginData).expect(200);
  ...
  describe('POST /api/user/login', () => {
    it('should login a user', async () => {
      ...
      const response = await request(server).post('/api/user/login').send(loginData).expect(200);
  ...
  describe('GET /api/user/:id', () => {
    it('should get a user by id', async () => {
      ...
      const response = await request(server).get(`/api/user/${user.id}`).set('Cookie', jwtToken).expect(200);
